...
#main data
library(readxl)
data <- read_excel("data_pro_revisado.xlsx")
...
summary_final <- rbind(summary1_clean, linha_total)


library(writexl)
write_xlsx(summary_final, "Tables/Table S1_species.xlsx")
...
serie <- ts(n,start=c(2017,1),frequency=12) #transformando dados em série temporal
...
#encontrando outliers/encalhes em massa
...
write_xlsx(df_outliers_all_2, "Tables/outliers.xlsx")

# Extrair os componentes da decomposição STL
...
# Gráfico 3: Componente de tendência
...
# Gráfico 4: Componente residual (barras verticais)
...
ggsave("Figures/Figure 2_stl Brazil.tiff", stl_Brazil, width = 10, height = 8, dpi = 300)
...
yr <- factor(Year)
tapply(n,yr, mean)
model1 <- lm(n~index+sin(time*2*pi)+cos(time*2*pi)) #index nao significativo, vamos fazer o modelo misto com e sem índice
summary(model1)
...
# Completar com os dias faltantes e preencher com 0
...
write_xlsx(merged_tibble, "Tables/Table 1_deposition rate.xlsx")
...
analise_changepoint <- function(data, ano, mes, medias_mensais) {
  ...
  # Filtrar dados para ano e mês
  dados_filtrados <- data %>%
    filter(Year == ano, Month == mes)
  
  # Garantir que Date esteja no formato Date
  ...
  # Criar sequência de datas para o mês inteiro
  ...
  # Completar com zeros onde não tem ocorrência
  ocorrencias_por_dia <- todas_datas %>%
    left_join(ocorrencias_por_dia, by = "Date") %>%
    mutate(ocorrencias = replace_na(ocorrencias, 0))
  
  # Salvar objetos no ambiente global
  ...
  # Extrair limiares do medias_mensais
  ...
  # médias por segmento (igual ao plot do changepoint)
...
#### criar coasline linearizada para análise espacial ####
...
#Converte seus pontos para objeto espacial (WGS84).
data_all_sf <- st_as_sf(data_temporal,
                        coords = c("Long", "Lat"),
                        crs = 4326)

#Transforma para UTM (SIRGAS 2000 / Brazil Albers Equal Area).
data_all_utm <- st_transform(data_all_sf, 5880)

brasil_utm <- st_transform(brasil, 5880)
coast_br <- st_boundary(brasil_utm) #extrair o contorno
coast_lines <- st_cast(coast_br, "LINESTRING") #transformar em linha
...
calc_isopleth_metrics <- function(dens_obj, evento_df, coast_line, level, event_name){
  ...
  # Recortar segmento corretamente
...
#### funçao densidade kernel ao longo da costa ####

calc_kde_coast <- function(evento, coast_line){
  
  # converter para sf e projetar
  evento_sf <- st_as_sf(
    evento,
    coords = c("Long", "Lat"),
    crs = 4326
  ...
  # converter para proporção ao longo da linha
...
# Encontrar índices correspondentes no eixo X (que é 1:n)
...
# Desenhar retângulo sobre o gráfico já existente
...
if (!dir.exists("Species_MME")) {
  dir.create("Species_MME")
}

write_xlsx(summary_epidemio_final,
           "Species_MME/MME 1_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/Brazil 1_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/MME 2_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/MME 3_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/MME 4_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/Brazil 2_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/MME 5_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/MME 6_summary_epidemio.xlsx")
...
write_xlsx(summary_epidemio_final,
           "Species_MME/MME 7_summary_epidemio.xlsx")
...
ggsave("Figures/Figure 3_changepoint analysis.tiff", 
       final_plot_changepoint, 
...
ggsave("Figures/Figure 4_density analysis.tiff",
       final_plot_density,
...
final_plot_MME <- (brazil_MME_1 | brazil_plot1 | brazil_MME_2) /
  ...
  plot_layout(
    heights = c(1, 1, 1),   # 3 linhas com altura igual
    widths = c(1, 1, 1)     # 3 colunas iguais
  ) &
  theme(
    legend.position = "right",
    legend.justification = c("right", "top"),
...
ggsave("Figures/Figure 5_MMEs.tiff", final_plot_MME, width = 12, height = 10, dpi = 300)
...
write_xlsx(hotspots_all, "Tables/Table 3_hotsplots Brazil.xlsx")
...
event_colors <- c(
  "June 2017"                  = "#F94144",  # vermelho
  "May 2023"                   = "#9D4EDD",  # roxo
  "May and June 2024"          = "#F8961E",  # laranja
  "May 2019 - 1"               = "#F15BB5",  # azul
  "May 2019 - 2"               = "#F15BB5",  # ciano (diferente do azul!)
  "September 2018"             = "#9C6644",  # marrom
  "September and October 2020" = "#3A86FF",  # rosa
  "October and November 2022"  = "#43AA8B"   # verde
)
...
offset_table <- data.frame(
  ...
  dx = c( 0,        # June 2017 (na costa)
          25000,    # May 2023 (mais afastado)
          50000,    # May and June 2024 (ainda mais)
          0,        # May 2019 - 1 (na costa)
          0,
          25000,    # May 2019 - 2
          50000,    # September 2018
          75000    # September and October 2020
          ),   # October and November 2022
...
ggsave(filename = "Figures/Figure 6_hotspots Brazil.tiff",
       plot = hotspots_brazil,
       width = 8, height = 7,
       dpi = 300)
...
# vetor para guardar número de picos
n_peaks <- numeric(length(bw_vals))

# loop para calcular densidade
